...
export default function CrewTable({ crews, onEdit, onDelete }: CrewTableProps) {
    ...
    <div className="bg-white rounded-xl sm:rounded-2xl border border-gray-100 shadow-lg">
      ...
      <div className="p-4 sm:p-6 border-b border-gray-100">
        <div className="flex flex-col space-y-4 md:flex-row md:items-center md:justify-between md:space-y-0">
          <h2 className="text-lg sm:text-xl font-semibold text-gray-900">Crew Members</h2>
        ...
        <table className="w-full">
          ...
          <tbody className="divide-y divide-gray-200">
            {filteredCrews.map((crew) => (
              <tr key={crew.id} className="hover:bg-gray-50 transition-colors duration-200">
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getStatusBadge(crew.status)}`}>
                    {crew.status.replace('_', ' ').replace(/\b\w/g, l => l.toUpperCase())}
                  </span>
                </td>
      ...
      <div className="lg:hidden">
        <div className="space-y-3 p-4">
          {filteredCrews.map((crew) => (
            <div key={crew.id} className="bg-gray-50 rounded-lg p-4 border border-gray-200">
              ...
              <div className="grid grid-cols-2 gap-3 text-xs">
                ...
                <div>
                  ...
                  <p className="font-medium text-gray-900">{new Date(crew.joinDate).toLocaleDateString()}</p>
                </div>
              </div>
            </div>
          ))}
